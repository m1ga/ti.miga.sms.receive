...
#define TAG "ti.miga.sms.receive"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tisms_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tisms.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TismsBindings::lookupGeneratedInit(
...
static void Tisms_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tisms_getBinding)->GetFunction());
}

static void Tisms_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TismsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TismsBinding = {
	"ti.miga.sms.receive",
	Tisms_init,
	Tisms_dispose
};
...
Java_ti_miga_sms_receive_TismsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.miga.sms.receive", &TismsBinding);
	titanium::KrollBindings::addExternalLookup(&(::TismsBindings::lookupGeneratedInit));
}
